...
(function(){
  ...
  window.__revSetMode = function(m){
    /* جدول درآمد همیشه همه کلبه‌ها را برای بازه انتخابی نشان می‌دهد؛ حالت فقط در بادج کشیدن کاربرد دارد */
    try { if (m === 'single' || m === 'free') localStorage.setItem('radar_mode_v1', m); } catch (e) {}
    applyRange();
  };
  window.__revApplyRange = function(f, t, mode, rids){
    curFrom = f; curTo = t;
    if (mode === 'single' || mode === 'free'){ try { localStorage.setItem('radar_mode_v1', mode); } catch (e) {} }
    applyRange();
  };
})();
</script>
