...
(function () {
  var KEY = 'radar_edits_v2';
  var LEGACY_KEY = 'radar_edits_v1';
  var EDITS = {};
  try { EDITS = JSON.parse(localStorage.getItem(KEY) || '{}') || {}; } catch (e) { EDITS = {}; }
  /* مهاجرت از v1: ویرایش‌های قدیمی بدون timestamp حفظ می‌شوند */
  try {
    if (!Object.keys(EDITS).length) {
      var legacy = JSON.parse(localStorage.getItem(LEGACY_KEY) || '{}') || {};
      Object.keys(legacy).forEach(function (k) {
        if (legacy[k] && typeof legacy[k] === 'object')
          EDITS[k] = { s: legacy[k].s, p: legacy[k].p, t: 0 };
      });
      if (Object.keys(EDITS).length) saveEdits();
    }
  } catch (e) {}

  /* ---------- server/file sync (cross-browser persistence) ----------
     مشکل: localStorage برای هر origin جداست — file:// و localhost:8080 و
     github.io سه حافظه جدا دارند؛ کاربر در یکی ویرایش می‌کند و در دیگری
     نمی‌بیند. راه‌حل: ذخیره مرکزی data/radar/manual-edits.json.
       - http(s)://localhost → GET/POST /api/edits (سرور محلی)
       - github.io           → GET manual-edits.json (اسنپ‌شات bake شده توسط
         build_radar_dashboard + push کرون؛ POST آنجا کار نمی‌کند و بی‌صدا
         رد می‌شود)
       - file://             → فقط localStorage
     ادغام بر اساس t (timestamp): رکورد جدیدتر برنده است. */
  var SYNC_URL = null, CAN_POST = false;
  try {
    var proto = location.protocol;
    if (proto === 'http:' || proto === 'https:') {
      if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
        SYNC_URL = '/api/edits'; CAN_POST = true;
      } else {
        SYNC_URL = 'manual-edits.json'; CAN_POST = false;
      }
    }
  } catch (e) {}

  function mergeServer(serverEdits) {
    if (!serverEdits || typeof serverEdits !== 'object') return false;
    var changed = false;
    Object.keys(serverEdits).forEach(function (k) {
      var se = serverEdits[k];
      if (!se || typeof se !== 'object') return;
      var le = EDITS[k];
      var st = se.t || 0, lt = le ? (le.t || 0) : -1;
      if (!le || st > lt) { EDITS[k] = se; changed = true; }
    });
    return changed;
  }
  ...
  function pullRemote() {
    ...
    try {
      fetch(SYNC_URL, { cache: 'no-store' })
        ...
        .then(function (data) {
          var payload = data && data.edits ? data.edits : data;
          if (mergeServer(payload)) { saveEdits(); applyAll(); updCount(); refreshRevenue(); }
        })
        .catch(function () {});
    } catch (e) {}
  }
  ...
  function saveEdits() {
    ...
    try { localStorage.setItem(LEGACY_KEY, JSON.stringify(EDITS)); } catch (e) {}
  }
  ...
  function save() { saveEdits(); pushRemote(); }
  ...
  function openPop(td) {
    ...
    /* ثبت فوری: هر انتخاب/تغییری همان لحظه در localStorage ذخیره می‌شود */
    function commitEdit() {
      var p = faToEn(priceInput.value).replace(/[^\d]/g, '');
      var entry = {};
      if (selStatus) entry.s = selStatus;
      if (p !== '') entry.p = parseInt(p, 10);
      if (entry.s || entry.p !== undefined) { entry.t = Date.now(); EDITS[keyOf(rid, d)] = entry; }
    ...
    for (var i = 0; i < sBtns.length; i++) {
      sBtns[i].onclick = (function (b) {
        return function () {
          ...
          b.classList.add('sel');
          commitEdit(); /* کلیک روی وضعیت = همان لحظه ثبت */
        };
      })(sBtns[i]);
    }
    priceInput.addEventListener('change', commitEdit); /* قیمت هنگام ترک فیلد/Enter ثبت می‌شود */
    priceInput.addEventListener('keydown', function (e) { if (e.key === 'Enter') commitEdit(); });
    pop.querySelector('.ep-save').onclick = function () { commitEdit(); closePop(); };
  ...
  applyAll();
  updCount();
  pullRemote(); /* ویرایش‌های ذخیره‌شده مرکزی را بکش (localhost / Pages) */
})();